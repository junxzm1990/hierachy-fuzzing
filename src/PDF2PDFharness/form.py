...
class PDF_FORM_STRU() : 
    ...
    def form_type(self, form, form_id) :
        ...
        cnt = 0 
        if str(form.fieldType) == '/Tx' :
            self.form_type_ret[form_id][cnt] = 'text'
        elif str(form.fieldType) == '/Btn' :
            if form.kids != None :
                self.form_type_ret[form_id][cnt] = "radio_" 
            else :
                if str(form.flags) == '0' :
                    self.form_type_ret[form_id][cnt] = "checkbox"
                else :
                    self.form_type_ret[form_id][cnt] = "button"
        elif str(form.fieldType) == '/Ch' :
            self.form_type_ret[form_id][cnt] = 'choice_8'
        elif str(form.fieldType) == '/Sig' :
    ...
    def form_parse(self) :
        ...
        for i in self.forms :
            ...
            self.maga_info[num_form] = dict()
            # update each attributes
            self.form_type(self.forms[i], num_form)
            # adding attributues to current handling form in maga_info
            self.maga_info[num_form] = self.form_type_ret[num_form]
            num_form += 1

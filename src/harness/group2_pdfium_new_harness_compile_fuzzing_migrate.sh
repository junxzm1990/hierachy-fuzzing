...
mkdir $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/

mkdir $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/queue/
...
then
        ...
        pre_cnt=0
       # mkdir $EVAL_BIN/result/group2_with_split_16_$DATE/$top_rank$DATE/ 
       # mkdir $EVAL_BIN/result/group2_with_split_16_$DATE/$top_rank$DATE/queue/ 
        ...
        do
                ...
                if [[ $increment_bool == 1 ]]; then
                        ...
                        do
                                if grep -q "$i" $EVAL_BIN/result/group2_with_split_16_$DATE/$top_rank$DATE/done_seeds
                                then
					...
					if [ "$I" -gt "$O" ]; then
						...
						mv $OUT_DIR/pdf_gen_trim_out/* $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/queue/$name
					else 
                                        	## 3.2.2 RENAMING : rename reduced size PDFs
						len=${#pre_cnt}
						bond=`expr 5 - $len` 
						...
						mv $OUT_DIR/pdf_gen_trim_in/* $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/queue/$name

					fi
					rm -rf $OUT_DIR/pdf_gen_trim_in/
					rm -rf $OUT_DIR/pdf_gen_trim_out/

                                        echo $i >> $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/done_seeds 
		...
		#	mkdir $EVAL_BIN/result/group2_with_split_16_$DATE/$top_rank$DATE/queue/ 
			...
			do
				...
				if [[ $increment_bool == 1 ]]; then
					for i in $OUT_DIR/pdf_gen/* 
					do 
						if grep -q "$i" $EVAL_BIN/result/group2_with_split_16_$DATE/harness_gen/done_seeds
						then
							echo $i" has been migrated" 
						else
							echo "NEW SEED : "$i

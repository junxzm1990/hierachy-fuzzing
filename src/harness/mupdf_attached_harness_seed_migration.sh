...
while true; do
	...
	if [ -z "$(ls -A $MAIN_GROUP/result/test_run_$DATE/harness_gen/)" ]; then
		...
		for harness_gen_seed in $MAIN_GROUP/result/test_run_$DATE/harness_gen/*;
		do
		...
		for obj_exchange_seed in $MAIN_GROUP/result/test_run_$DATE/obj_exchange/*;
		do
		...
		for obj_entry_seed in $MAIN_GROUP/result/test_run_$DATE/obj_entry_mutation/*;
		do

...
mkdir $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/
mkdir $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/queue
mkdir $OUT_DIR/entry_gen/
while true; do 
	pre_cnt=`ls $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/queue |wc -l`
#	for queue in `find $EVAL_BIN/result/group1_with_split_16_$DATE/ -name "queue" | grep -v -e "afl_" -e "obj_"`; do 
#	
		...
		for seed in $OUT_DIR/pdf_gen/*; do
			...
			for i in $OUT_DIR/entry_gen/*; do
				...
				if [ -z "$(ls -A $OUT_DIR/entry_gen_trim_out/)" ]; then
                       		         ...
                       		         mv $i $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/queue/$name
                       		 else
                       		         ...
                       		         if [ "$I" -gt "$O" ]; then
                       		                 ...
                       		                 mv $OUT_DIR/entry_gen_trim_out/* $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/queue/$name
                       		         else
                       		                 ## 3.2.2 RENAMING : rename reduced size PDFs
                       		                 len=${#pre_cnt}
                       		                 bond=`expr 5 - $len`
                       		                 ...
                       		                 mv $OUT_DIR/entry_gen_trim_in/* $EVAL_BIN/result/group1_with_split_16_$DATE/obj_entry_mutation/queue/$name

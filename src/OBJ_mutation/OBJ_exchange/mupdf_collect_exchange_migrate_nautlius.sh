...
while true; do
	...
	if  [ -z "$(ls -A $EVAL_BIN/result/nautilus_test_run_$DATE/afl_S_0/queue/)" ]; then
		...
		echo "NO NEW SEEDS YET-LET's WAIT"
        	...
        	pre_cnt=`ls $EVAL_BIN/result/test_run_$DATE/obj_nautilus/queue/ | wc -l`
        	mkdir $OUT_DIR/tmp_obj_exchange/

        	python2.7 $SRC/src/OBJ_mutation/OBJ_exchange/new_generator.py $SRC/src/OBJ_mutation/OBJ_exchange/class_obj_hongbin.json $EVAL_BIN/result/test_run_$DATE/afl_S_0/queue/ $EVAL_BIN/result/test_run_$DATE/nautilus/queue/

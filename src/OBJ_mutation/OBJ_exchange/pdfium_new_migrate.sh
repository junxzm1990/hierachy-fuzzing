...
while true; do 
	...
	pre_cnt=`ls $EVAL_BIN/result/group1_with_split_16_$DATE/obj_exchange/queue/ | wc -l`
	# migrate new generated seeds to queue
        if [ -z "$(ls -A $OUT_DIR/tmp_obj_exchange/)" ]; then 
	        ...
	        for new in $OUT_DIR/tmp_obj_exchange/*; do
				...
				mv $new $EVAL_BIN/result/group1_with_split_16_$DATE/obj_exchange/queue/$name
         		else
				# Compare trimed and untrimed, which one is smaller
                                ...
                                if [ "$I" -gt "$O" ]; then
                                        ...
                                        mv $OUT_DIR/tmp_obj_exchange_trim_out/* $EVAL_BIN/result/group1_with_split_16_$DATE/obj_exchange/queue/$name
                                else 
                                        ## 3.2.2 RENAMING : rename reduced size PDFs
                                        len=${#pre_cnt}
                                        bond=`expr 5 - $len` 
                                        ...
                                        mv $OUT_DIR/tmp_obj_exchange_trim_in/* $EVAL_BIN/result/group1_with_split_16_$DATE/obj_exchange/queue/$name
